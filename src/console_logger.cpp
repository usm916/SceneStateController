...
void ConsoleLogger::print_banner() {
  ...
  out_.println("  SCENE <IDLE|MOVE|ARRIVED|ERROR>");
...
void ConsoleLogger::print_mode_usage() {
  ...
  out_.print("Loop frame us (min/max/recent10)=");
  out_.print(frame_timing_min_us());
  out_.print("/");
  out_.print(frame_timing_max_us());
  out_.print("/[");
  const uint8_t recent_count = frame_timing_recent_count();
  for (uint8_t i = 0; i < recent_count; ++i) {
    if (i > 0) out_.print(",");
    out_.print(frame_timing_recent_us(i));
  }
  out_.println("]");
}
